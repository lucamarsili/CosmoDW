...
VVD SplinePath::pts_at_dist(VD X)
{
    ...
    #def pts_at_dist(double x, int deri):
     #   return x
...
def fullTunneling(path_pts, V, dV, maxiter=20, fixEndCutoff=.03,
                  ...
                  tunneling_class=tunneling1D.Kink1D,
    ...
    rtuple = namedtuple("fullTunneling_rval",
                        "profile1D Phi dPhi action fRatio saved_steps")
    Phi = path.pts(profile1D.Phi)
    dPhi = path.pts(profile1D.dPhi)
   
    action = tobj.findAction(profile1D)

    ## all well, but we need to move from the 1Dprofile to a multifield profile
   
    return rtuple(profile1D, Phi,dPhi, action, fRatio, saved_steps)
