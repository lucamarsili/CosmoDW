...
class Kink1D:
    ...
    It is taken from SingleFieldInstanton in Tunneling1D from CosmoTransition.
    Most users will probably be primarily interested in the functions
    :func:`findProfile` and :func:`findAction`.
    """
    #removed alpha from here, not a phase transition
    def __init__(self, phi_left, phi_right, V,
                 dV, d2V=None, phi_eps=1e-6, 
                 phi_bar=None, rscale=None):
        #Commenting each changes from the original code from here below:#
        self.phi_left, self.phi_right = phi_left, phi_right #changed here
        self.V = V
        self.dV = dV
        #here instead of the phi in which we have meta and abs vacuum, we have phileft 
        #and phi right, the boundary conditions to be imposed
        if dV is not None:
            self.dV = dV
        if d2V is not None:
            self.d2V = d2V
        ...
        if rscale is None:
            ...
            self.rscale = rscale
        self.phi_eps = phi_eps * abs(phi_left - phi_right)
    ...
    def findBarrierLocation(self):
        ...
        """
        #here the algorithm for the domain walls is simpler

        #translating in python the algorithm in Kink1D.cpp
        #1. Find the absolute and metastable minimum from the boundary conditions
        if (self.phi_right <= self.phi_left) :
                self.phi_abs = self.phi_right
                self.phi_meta = self.phi_left
                self.match_left_meta = True
        else :
                self.phi_abs = self.phi_left
                self.phi_meta = self.phi_right
                self.match_left_meta = False
        
        #Find the position where self.phi_bar == self.phi_meta
        if (np.abs(self.V(self.phi_abs)-self.V(self.phi_meta))<1e-3) :
            #If the V at abs/meta point are quite close, just set the barrier at phi_abs;
            self.phi_bar = self.phi_abs
        #otherwise, binary search
        
        phi_tol = abs(self.phi_meta - self.phi_abs) * 1e-12
        V_phimeta = self.V(self.phi_meta)
        phi1 = self.phi_meta
        phi2 = self.phi_abs
        phi0 = 0.5 * (phi1+phi2)
    ...
    #In c++ this method above finds also the top of the barrier, we try to remain
    #near to the methods of CosmoTransition
    def findRScale(self):
        R'''
        '''
        phi_tol = abs(self.phi_bar - self.phi_meta) * 1e-6
        x1 = min(self.phi_bar, self.phi_meta)
        x2 = max(self.phi_bar, self.phi_meta)
        ...
        Vtop = self.V(phi_bar_top) - self.V(self.phi_meta)
        xtop = phi_bar_top - self.phi_meta
        #until here just found the top of the potential 
        #follow c++ code from here below 
        ...
        zscale = np.abs(xtop)/np.sqrt(np.abs(2*Vtop))
        phiscale = np.abs(self.phi_abs)
        '''
        if Vtop <= 0:
        ...
        '''
        return zscale
        #important thing, in c++ scale only use for 1 parameter, but is better to do it at a lower level

    ####IMPORTANT, DW CASE IS SIMPLER WE DON'T USE EXACTSOLUTION METHOD,SINCE WE CAN DIRECTLY COMPUTE THE 
    #EQ FROM Z = -INFINITY TO Z = +INFINITY, EXACT SOLUTION USED ONLY FOR R<RMIN, NOT OUR PROBLEM
    ...
    def equationOfMotion(self, y, r):
        ...
        """
        #we don't perform any scaling at the moment
        #also alpha is zero so these are the equations
        return np.array([y[1], self.dV(y[0])])

    #Core function
    _integrateProfile_rval = namedtuple(
        "integrateProfile_rval", "r y convergence_type")
    def integrateProfile(self, r0, y0, y0desired, dr0,
                         epsfrac, epsabs, drmin, rmax, *eqn_args):
        R"""
        We need to generalize it a bit, since we don't need to go only from abs to meta, but we need it after also
        to go from top to meta. 
        ...
        while True:
            ...
            elif ((np.abs(y1 - y0desired)) < 3*epsabs).all():
                r,y = r1,y1
                convergence_type = "converged"
                break

            elif y1[1]*ysign > 0 or (y1[0]-y0desired[0])*ysign < 0:
                f = cubicInterpFunction(y0, dr*dydr0, y1, dr*dydr1)
                if(y1[1]*ysign > 0):
                    # Extrapolate to where dphi(r) = 0
                    #need to change this interpolation following the c++ code
                    #print(f(0))
                    #print(f(1))
                    ...
                    x = optimize.brentq(lambda x: f(x)[0]-y0desired[0], 0,1)
        ...
        if (abs(y - y0desired) < 3*epsabs).all():
            convergence_type = "converged"
    ...
    def integrateAndSaveProfile(self, R, y0, dr,
        ...
        """
        Is the same as cosmotransition, so just modify findProfile to adapt it to
        DW situation ad then perform the validation 
        """
        N = len(R)
        R, r0 = np.array(R), R[0]
    ...
    def findProfile(self, xguess=None, xtol=1e-5, phitol=1e-5, dphitol= 1e-12,
                    thinCutoff=.1, npoints=500, rmin=1e-4, rmax=1e4,
                    max_interior_pts=None):
        R"""#set 1e-5 otherwise RK is not precise enough for the trivial DW.
        Things to modify: 
        1. We don't need to use x, just the fields are ok
        """
        #Define the guess from Z2 domain walls
        dphi0_initial_guess = np.sqrt(2*np.abs(self.V(self.phi_bar_top)-self.V(self.phi_abs)))
        #print(np.sqrt(2*np.abs(self.V(self.phi_bar_top)-self.V(self.phi_abs))))
        #print(dphi0_initial_guess)
        #starting point is at phi0, above the initial guess for the derivative
        if self.phi_bar_top < self.phi_abs:
            dphi0_initial_guess = dphi0_initial_guess
        else:
            dphi0_initial_guess = -dphi0_initial_guess
        ## modify from here
        #1. We start from phi_bar
        '''
        # Set x parameters
        no need to introduce the variable x
        xmin = xtol*10
        xmax = np.inf
        if xguess is not None:
            x = xguess
        else:
            x = -np.log(abs((self.phi_bar-self.phi_abs) /
                            (self.phi_meta-self.phi_abs)))
        xincrease = 5.0
        ''' 
        ...
        This above is the c++ code, a bit different than cosmo.
        1. We don't need to introduce ydesired, we just use phi_abs in integrateProfile and we don't have
        to pass it to the function
        2. We need to introduce the starting point phi0, which before was x so we are going to translate 
        the lines for phi0 and dphi0 below.
        3. We don't use initial conditions so we just define phi0 and dphi0 out of the while cycle, as in the 
        c++ code.
        '''
        # Set the phi parameters
        phi0 = self.phi_bar_top
        #print("phi bar 0")
        #print(phi0)
        ...
        dphi0 = dphi0_initial_guess*dphi0_rel
        #basically one now
        #we just act on the initial condition of the derivative, trial and error until it converges
        delta_phi = self.phi_meta - self.phi_abs
        ...
        rf = None
        #print(self.phi_abs)
        y0desired = np.array([self.phi_abs,0])
        #2. Shooting until it converges
        while True:
            #we can safely take r0 to be zero always
            r0 = 1e-4
            y0 = np.array([phi0, dphi0])
            rfabs, yf, ctype = self.integrateProfile(r0, y0,y0desired, *integration_args)
            # Check for overshoot, undershoot
            if ctype == "converged":
                break
            elif ctype == "undershoot":  # phi0 is too low
                dphi0_min_rel = dphi0_rel #for example instead of 0.9 now is 1, so the mult factor is higher than one
                #"faster" initial condition, less undershooting
                dphi0_rel = (dphi0_min_rel+dphi0_max_rel)/2.0
                dphi0 = dphi0_rel * dphi0_initial_guess
            elif ctype == "overshoot":  # x is too high
                dphi0_max_rel = dphi0_rel #for example instead of 1.1 now is 1, so the mult factor is lower than one
                #"slower" initial condition, less overshooting
                dphi0_rel = (dphi0_min_rel+dphi0_max_rel)/2.0
                dphi0 = dphi0_rel * dphi0_initial_guess
            # Check if we've reached xtol
            if np.abs((dphi0_max_rel-dphi0_min_rel)) < dphitol:
                break
        #fixed until here, save the output in a proper way
        dphi0_solution = dphi0
        #this is the main difference from the PT code, we rewrite the c++ code step by step mantaining 
        #the convention of cosmoTransition
        ydesired = np.array([self.phi_meta,0])
        ymeta = np.array([self.phi_bar_top, -dphi0_solution])
        rfmeta, yfmeta, ctypemeta = self.integrateProfile(r0, ymeta,ydesired, *integration_args)
        if ctypemeta == "undershoot": 
            print("In findProfile: converge in absolute minimum side but undershoot in meta-stable side. Either the potential has a Z2 symmetry but the RK precsion is not enough, or something wrong in the potential!")
        #above: integrate back from top to meta to compute the rscale in the meta side, in the case we have a non symmetric
        #domain
        #save output below
        # Integrate a second time, this time getting the points along the way
        
        #1. From 0 to absolute value (right)
        R_abs = np.linspace(0, rfabs, npoints)
        y0abs = np.array([self.phi_bar_top, dphi0_solution])
        profile_abs = self.integrateAndSaveProfile(R_abs, y0abs, dr0,
                                               epsfrac, epsabs, drmin)

        #2. From 0 to meta value (left)
        R_meta = np.linspace(0, rfmeta, npoints)
        y0meta = np.array([self.phi_bar_top, -dphi0_solution])
        profile_meta = self.integrateAndSaveProfile(R_meta, y0meta, dr0,
                                               epsfrac, epsabs, drmin)
        #from here has to be coherent and working!!!
        #3. Combine the two halves
        #reverse the absolute value
        Rmeta = profile_meta.R 
        Phimeta = profile_meta.Phi
        dPhimeta = profile_meta.dPhi

        Rabs = profile_abs.R 
        Phiabs = profile_abs.Phi
        dPhiabs = profile_abs.dPhi

        Rerrabs = profile_abs.Rerr
        Rerrmeta = profile_meta.Rerr
        Rerr_final = []
        
        if self.match_left_meta: 
            R_final = Rabs[1:]
            Phi_final = Phiabs[1:]
            dPhi_final = dPhiabs[1:]
           
            #reverse meta part 
            Rmeta = -Rmeta
            dPhimeta = -dPhimeta
            R_final = np,concatenate((R_meta[::-1][1:],R_final))
            Phi_final = np.concatenate((Phimeta[::-1][1:] , Phi_final))
            dPhi_final = np.concatenate((dPhimeta[::-1][1:] , dPhi_final))
          
            #rebuild profile
            rval_final = (R_final,)+(Phi_final,) + (dPhi_final,)+(Rerr_final,)
            profile = self.profile_rval(*rval_final)
        else: 
            R_final = Rmeta[1:]
            Phi_final = Phimeta[1:]
            dPhi_final = dPhimeta[1:]
           
            #reverse meta part 
            Rabs = -Rabs
            dPhiabs = -dPhiabs

            R_final = np.concatenate((Rabs[::-1][1:], R_final))
            Phi_final =np.concatenate((Phiabs[::-1][1:],Phi_final))
            dPhi_final = np.concatenate((dPhiabs[::-1][1:] , dPhi_final))
    ...
    def evenlySpacedPhi(self, phi, dphi, npoints=1000, k=1, fixAbs=True):
        """
        ...
        """
        if fixAbs is True:
            phi = np.append(self.phi_abs, np.append(phi, self.phi_meta))
            dphi = np.append(0.0, np.append(dphi, 0.0))
        else:
            phi = np.append(phi, self.phi_meta)
        ...
        if fixAbs:
            p = np.linspace(self.phi_abs, self.phi_meta, npoints)
        else:
            p = np.linspace(phi[i][0], self.phi_meta, npoints)
...
class SingleFieldInstanton:
    ...
    def __init__(self, phi_absMin, phi_metaMin, V,
                 dV=None, d2V=None, phi_eps=1e-3, alpha=0,
                 phi_bar=None, rscale=None):
